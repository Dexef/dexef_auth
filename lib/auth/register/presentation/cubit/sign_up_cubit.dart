...
import 'package:auth_dexef/auth/register/presentation/cubit/sign_up_states.dart';
...
import '../../../login/domain/useCase/confirm_email_use_case.dart';
...
class SignUpCubit extends Cubit<SignUpInitialState> {
  SignUpCubit(
  ...
  ////////////////// confirm email
  // is Email Exist API
  confirmEmail(String email) async {
    emit(LoadingState());
    final result = await confirmEmailUseCase!(email);
    result.fold((failure) => emit(NetworkError(failure.errorMessage)),
        (confirmEmail) {
      checkEmailExist(confirmEmail.data!.isEmailExist!);
    });
  }

  /////////////// check email exist
  void checkEmailExist(bool isEmailExists) {
    if (isEmailExists) {
      emit(ConfirmEmailFailed());
    } else {
  ...
  signUP(String email, String name, String phoneNumber, String password,
      String countryCode) async {
    emit(LoadingState());
    final result = await signUpUseCase!(
        email, name, phoneNumber, password, countryCode, "default");
    result.fold((failure) {
      errorMessage = failure.errorMessage;
      emit(NetworkError(failure.errorMessage));
    }, (signup) {
      if (signup.isSuccess == true) {
        signUpEntity = signup;
        if (signup.data!.status == 4) {
          saveSignUpDataInShared(signup);
          emit(SignUpSuccess(signup.data!.mobileId!.toInt()));
        } else {
          emit(SignUpUnSuccess(
              AppLocalizations.of(currentContext)!.translate('an_error')));
        }
      } else {
        errorMessage = signup.errors!.map((e) => e.message).join(', ');
        emit(SignUpFailed(signup.errors!.first.message!, signup.errors!));
      }
    });
  }
  ...
  void saveSignUpDataInShared(SignUpEntity signup) {
